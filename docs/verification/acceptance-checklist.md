...
| --- | --- | --- | --- |
| Notification permission and banners | Reminders can visibly alert the user through macOS | User-authorized notification permission flow, observed banner/list delivery for one-time and recurring reminders, denied-permission behavior, Focus behavior, restart/reboot recovery, and documentation of rolling-window limits. Use the [native notification acceptance procedure](native-notification-acceptance.md). | Open. Existing tests cover planning, persistence, and reconciliation with fake OS boundaries, not observed banner delivery. |
