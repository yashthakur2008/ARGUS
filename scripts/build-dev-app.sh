...
while IFS= read -r rpath; do
  case "$rpath" in
    ...
    /*) /usr/bin/install_name_tool -delete_rpath "$rpath" "$APP/Contents/MacOS/ARGUS" ;;
  esac
done < <(/usr/bin/otool -l "$APP/Contents/MacOS/ARGUS" | /usr/bin/awk '/cmd LC_RPATH/{found=1; next} found && /path /{sub(/^ *path /, ""); sub(/ \(offset.*$/, ""); print; found=0}')
